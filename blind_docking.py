...
for folder_name in os.listdir():
    if os.path.isdir(folder_name):
       ...
       # To execute the command sed
        os.system("sed '/CONECT/,$d' protein_no_spaces.pdb > protein_no_spaces.pdbqt")
        os.system("sed -i '1,6d' protein_no_spaces.pdbqt")
        # To execute the command
        with open("protein_no_spaces.pdb", "r") as f:
           original_content = f.read()

        modified_content = original_content.replace('CONECT', '')
        modified_content = '\n'.join(modified_content.splitlines()[6:])
        modified_content = re.sub(r'(\.[^ ]{3})', r'\1 ', modified_content)
        ...
        #To move the file aux.txt
        source_file = os.path.join(path_root, "Seqs", "Docking", folder_name, "tmp", "aux.txt")
        destination_file = os.path.join(path_root, "Seqs", "Docking", folder_name, "aux.txt")
        os.rename(source_file, destination_file)

        # To read the file aux.txt and calculate the values
        with open(os.path.join(path_root, "Seqs", "Docking", folder_name, "aux.txt")) as file:
